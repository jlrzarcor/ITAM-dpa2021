...
import src.test.test_training as tst
...
class TaskTrainUnitTest(CopyToTable):
    ...
    def rows(self):
        ...
        os.system('echo ' + self.input().path + ',' + str(self.force3_err) + ' > src/test/trans_file.csv')

        # Objects returned by test
        marble_obj = tst.TestTrain()
        marble_obj.test_binary()
