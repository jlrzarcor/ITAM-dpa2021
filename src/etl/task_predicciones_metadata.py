...
# Requires...
from src.etl.task_predicciones_unit_test import TaskPredUnitTest


# ================================= LUIGI TASK ================================= #

class TaskPredictMeta(CopyToTable):
    ...
    def requires(self):
        return TaskPredUnitTest(self.bucket, self.prc_path, self.year, self.month, self.day, self.flg_i0_c1, self.avg_prec)
    ...
    port = pg['port']
    database = pg['dbname']
    table = 'metadata.predictions'
    
    # RDS database table columns
    columns = [("exec_date", "date"), ("exec_param", "json"), ("executer", "varchar"), ("num_new_preds", "int"),
               ("S3_store_path", "text")
              ]
